...
s32 zynq_write_dir(wheelConfig pos, dirState state){
	if (wheels[pos].state == state)
		return XST_SUCCESS;

	if ( (wheels[pos].state & 0b10) != (state & 0b10) )
		XGpioPs_WritePin(&gpio_emio, wheels[pos].pins[FW], (u32)(state >> 1));
	if ( (wheels[pos].state & 0b01) != (state & 0b01) )
		XGpioPs_WritePin(&gpio_emio, wheels[pos].pins[BW], (u32)(state & 0b01));

	wheels[pos].state = state;
}

dirState zynq_read_dir(wheelConfig pos){
	u32 pin_state_fw, pin_state_bw;
	dirState state;

	pin_state_fw = XGpioPs_ReadPin(&gpio_emio, wheels[pos].pins[FW]);
	pin_state_bw = XGpioPs_ReadPin(&gpio_emio, wheels[pos].pins[BW]);

	state = (pin_state_fw << 1) | pin_state_bw;
	wheels[pos].state = state;
